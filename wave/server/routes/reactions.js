...
router.post('/', auth, (req, res) => {
  ...
  const existing = queryOne(
    'SELECT id, emoji FROM reactions WHERE targetId = ? AND userId = ? AND targetType = ?',
  ...
  if (existing) {
    ...
    run('INSERT INTO reactions (targetId, targetType, userId, emoji) VALUES (?, ?, ?, ?)',
      [targetId, targetType, userId, emoji]);
    res.json({ removed: false, emoji });
  }
});
...
router.get('/:targetId/:targetType', auth, (req, res) => {
  ...
  const userReactions = query(
    ...
    [req.params.targetId, req.params.targetType, userId]
  );
