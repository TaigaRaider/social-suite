  ...
  try {
    ...
    contacts.forEach(c => {
      ...
      run(`INSERT INTO contacts (userId, contactUserId, name, phone, email, importedAt, syncedAt)
        ...
        [req.user.id, linkedUserId, c.name || 'Unknown', phone || null, email || null]);
      imported++;
    });
...
router.post('/contacts/sync', auth, (req, res) => {
  try {
    const contacts = queryAll('SELECT * FROM contacts WHERE userId = ? AND contactUserId IS NULL', [req.user.id]);
    let matched = 0;
...
router.delete('/contacts/:contactId', auth, (req, res) => {
  try {
    ...
    run('DELETE FROM contacts WHERE id = ? AND userId = ?', [contactId, req.user.id]);
    saveDB();
    res.json({ success: true });
  } catch (err) {
...
router.get('/search/messages', auth, (req, res) => {
  try {
    ...
    let sql = `SELECT m.*, u.username, u.firstName, u.lastName, g.name as groupName
               FROM messages m JOIN users u ON m.senderId = u.id JOIN groups g ON m.groupId = g.id
               WHERE m.groupId IN (SELECT groupId FROM group_members WHERE userId = ?)
               AND m.content LIKE ?`;
...
// Search groups
router.get('/search/groups', auth, (req, res) => {
  try {
    const q = req.query.q;
    const limit = Math.min(parseInt(req.query.limit) || 20, 50);
