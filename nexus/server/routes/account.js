...
router.delete('/', auth, async (req, res) => {
  try {
    ...
    run('DELETE FROM friendships WHERE userId = ? OR friendId = ?', [req.userId, req.userId]);
...
router.get('/export', auth, (req, res) => {
  try {
    ...
    const postLikes = query('SELECT * FROM likes WHERE userId = ?', [userId]);
    const comments = query('SELECT * FROM comments WHERE userId = ?', [userId]);
    const sentMessages = query('SELECT * FROM messages WHERE senderId = ?', [userId]);
    const receivedMessages = query('SELECT * FROM messages WHERE receiverId = ?', [userId]);
    const notifications = query('SELECT * FROM notifications WHERE userId = ? OR relatedUserId = ?', [userId, userId]);
    const friendships = query('SELECT * FROM friendships WHERE userId = ? OR friendId = ?', [userId, userId]);
