...
const DISMISS_KEY = 'lumina_profile_completion_dismissed';
...
export default function ProfileCompletion({ profile }) {
  ...
  const items = [
    ...
    { label: 'Followed someone', done: (profile.followingCount || 0) > 0, path: '/explore' },
  ];
    ...
    <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
      ...
      {items.map((item, i) => (
        <div key={i} onClick={() => { if (!item.done) navigate(item.path); }}
          style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', cursor: item.done ? 'default' : 'pointer', fontSize: 13, color: item.done ? 'var(--text-muted)' : 'var(--text-primary)' }}>
