...
router.get('/me', auth, (req, res) => {
  try {
    ...
    const postCount = queryOne('SELECT COUNT(*) as count FROM posts WHERE userId = ? AND replyToId IS NULL', [req.userId]);
    res.json({ ...user, followerCount: followerCount.count, followingCount: followingCount.count, postCount: postCount.count });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
router.get('/user/:id', auth, (req, res) => {
  try {
    const user = queryOne('SELECT id, username, email, firstName, lastName, bio, avatar, createdAt FROM users WHERE id = ?', [req.userId === parseInt(req.params.id) ? req.userId : req.params.id]);
    if (!user) return res.status(404).json({ error: 'User not found' });
    ...
    const postCount = queryOne('SELECT COUNT(*) as count FROM posts WHERE userId = ? AND replyToId IS NULL', [user.id]);
    const isFollowing = queryOne('SELECT id FROM follows WHERE followerId = ? AND followingId = ?', [req.userId, user.id]);
    res.json({ ...user, followerCount: followerCount.count, followingCount: followingCount.count, postCount: postCount.count, isFollowing: !!isFollowing });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
router.get('/search', auth, (req, res) => {
  try {
    ...
    const users = query('SELECT id, username, firstName, lastName, avatar, bio, status FROM users WHERE username LIKE ? OR firstName LIKE ? OR lastName LIKE ? LIMIT 20',
      [`%${q}%`, `%${q}%`, `%${q}%`]);
    res.json(users);
  } catch (err) {
    res.status(500).json({ error: err.message });
