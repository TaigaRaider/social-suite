...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const messageCount = queryOne('SELECT COUNT(*) as count FROM posts WHERE userId = ?', [req.userId]);
...
router.delete('/', auth, async (req, res) => {
  try {
    ...
    run('DELETE FROM poll_votes WHERE pollId IN (SELECT id FROM polls WHERE userId = ?)', [req.userId]);
    run('DELETE FROM poll_options WHERE pollId IN (SELECT id FROM polls WHERE userId = ?)', [req.userId]);
    run('DELETE FROM polls WHERE userId = ?', [req.userId]);
    run('DELETE FROM reactions WHERE postId IN (SELECT id FROM posts WHERE userId = ?)', [req.userId]);
    run('DELETE FROM reposts WHERE postId IN (SELECT id FROM posts WHERE userId = ?)', [req.userId]);
    run('DELETE FROM bookmarks WHERE postId IN (SELECT id FROM posts WHERE userId = ?)', [req.userId]);
    run('DELETE FROM posts WHERE userId = ?', [req.userId]);

    run('DELETE FROM likes WHERE userId = ?', [req.userId]);
    run('DELETE FROM reposts WHERE userId = ?', [req.userId]);
    run('DELETE FROM bookmarks WHERE userId = ?', [req.userId]);
...
router.get('/export', auth, (req, res) => {
  try {
    ...
    const polls = query('SELECT * FROM posts WHERE userId = ? AND type = ?', [userId, 'poll']);
    const pollVotes = query('SELECT pv.* FROM poll_votes pv JOIN posts p ON pv.pollId = p.id WHERE p.userId = ?', [userId]);
    const likes = query('SELECT * FROM likes WHERE userId = ?', [userId]);
    const reposts = query('SELECT * FROM reposts WHERE userId = ?', [userId]);
    const bookmarks = query('SELECT * FROM bookmarks WHERE userId = ?', [userId]);
    const follows = query('SELECT * FROM follows WHERE followerId = ? OR followingId = ?', [userId, userId]);
